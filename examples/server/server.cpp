...
static void maybe_change_model(llama_server_context &llama, const json &body)
{
    if (body.count("model") != 0) { // model was specified in request
        ...
        if (model_name != llama.params.model) {
            ...
            auto mparams = llama_model_params_from_gpt_params(llama.params);
            llama. model =
              llama_load_model_from_file(model_name.c_str(), mparams);
            if (!llama.model) {
                throw std::runtime_error("failed to load model\n");
            }
            auto cparams =
              llama_context_params_from_gpt_params(llama.params);
            llama.ctx = llama_new_context_with_model(llama.model, cparams);
            if (!llama.ctx) {
                llama_free_model(llama.model);
                llama.model = nullptr;
            ...
            llama.num_prompt_tokens = 0;
            llama.num_tokens_predicted = 0;
            llama.n_past = 0;
            llama.n_remain = 0;
            llama.ctx_sampling = nullptr;
            llama.truncated = false;
            llama.stopped_eos = false;
            llama.stopped_word = false;
            llama.stopped_limit = false;
            llama.multibyte_pending = 0;*/
        }
    }
}
...
int main(int argc, char **argv)
{
    ...
    svr.Post("/get_tokens", [&llama](const Request &req, Response &res)
        ...
        int bos = llama_token_bos(llama.ctx);
        int eos = llama_token_eos(llama.ctx);
        json data = json{
          {"begin_of_stream", bos},
          {"end_of_stream", eos},
          {"tokens", tokens}
