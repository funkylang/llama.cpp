...
BUILD_TARGETS = \
	...
	simple batched batched-bench save-load-state server funky_server gguf gguf-split eval-callback llama-bench libllava.a llava-cli baby-llama beam-search  \
...
ifdef LLAMA_HIP_UMA
	...
	HIPFLAGS    += --offload-arch=gfx1030
	HIPFLAGS    += --offload-arch=gfx1036
